...
class PrototypicalNetwork(nn.Module):
    ...
    def forward(self, input_batch: ProtoNetBatch):
        ...
        if self.config.distance_metric == "mahalanobis":
            ...
            logits2 = torch.mul(first_half, repeated_difference)
    
            print("logits2 = torch.mul(first_half, repeated_difference)")
            print(logits2)
            print(logits2.size())

            print("\n\n")
            print("second half of logits2")
            positive_logits = logits2[0,:,:]
            print(positive_logits)
            print(positive_logits.size())

            embeddings = positive_logits.tolist()

...
def evaluate_protonet_model_pred(
    model: PrototypicalNetwork,
    dataset: FSMolDataset,
    support_sizes: List[int] = [16, 128],
    num_samples: int = 5,
    ...
    def test_model_fn_pred(
        ...
        _, result_metrics = run_on_batches_pred(
            model,
            batches=pn_task_sample.batches,
            batch_labels=pn_task_sample.batch_labels,
            train=False,
    ...
    return eval_model(
        test_model_fn=test_model_fn_pred,
        dataset=dataset,
        train_set_sample_sizes=support_sizes,
        out_dir=save_dir,
        num_samples=num_samples,
